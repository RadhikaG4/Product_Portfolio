...
- Real copy is in for Work windows, Lab, hero intro and folder. Case study pages (`/work/<slug>/`) still use placeholder copy.
...
## Structure
- Home (`src/pages/index.astro`): hero (photo, glass intro, draggable resume folder), fade band, Work, the lab (side projects as dropdown rows), contact footer.
...
- Side projects: Novartis, Forus Health, AI Build Lab, MakeMyTrip. Paytm and Elective Planner are removed.
...
- `src/data/projects.js`: project names, dates, each Work window (`window`: lede, tags, overview, role, highlights, tools, results, learnings, hero, gallery of [image, caption]), optional `caseUrl` for an outside case study link, Lab rows (`text`), contact details.
- Work images: `public/work/*.webp` (hero 1440 wide, gallery 800 wide). Country Delight gallery image 4 is missing (placeholder).
...
- Footer: halftone panel (black dots fading to lab bg) with flowers and name.
...
- Resume link in menu (ask before publishing PDF).
